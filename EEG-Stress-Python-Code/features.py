...
def time_series_features(data):
    ...
    features = features.reshape([n_trials * n_secs, n_channels * features_per_channel])
    return features
...
def freq_band_features(data, freq_bands):
    ...
    for i, trial in enumerate(data):
        for j, second in enumerate(trial):
            psd = mne_f.compute_pow_freq_bands(sfreq, second, freq_bands=freq_bands)
            features[i][j] = psd
    features = features.reshape([n_trials * n_secs, n_channels * features_per_channel])
    return features
...
def hjorth_features(data):
    ...
    for i, trial in enumerate(data):
        for j, second in enumerate(trial):
            ...
            complexity_spect = mne_f.compute_hjorth_complexity_spect(sfreq, second)
...
def entropy_features(data):
    ...
    for i, trial in enumerate(data):
        for j, second in enumerate(trial):
            ...
            features[i][j] = np.concatenate([app_entropy, samp_entropy, spect_entropy, svd_entropy])
    features = features.reshape([n_trials * n_secs, n_channels * features_per_channel])
    return features